...
void	draw_wireframe(t_view *view)
{
	...
	while (++y < view->height)
	{
		...
		{
			if (x < view->width - 1)
				ft_drawline_3d(view, *(view->points[y][x]->view),
					*(view->points[y][x + 1]->view));
			if (y < view->height - 1)
				ft_drawline_3d(view, *(view->points[y][x]->view),
					*(view->points[y + 1][x]->view));
		}
	}
}
	...
	while (++y < view->height)
	{
		...
		while (++x < view->width)
		{
			ft_vec_mat_mult(view->points[y][x]->world, global,
				view->points[y][x]->view);
			view->points[y][x]->view->z = view->points[y][x]->local->z;
		}
	}
}
